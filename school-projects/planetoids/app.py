...
class Planetoids(GameApp):
    ...
    def start(self):
        ...
        if self._state == STATE_INACTIVE:
            ...
            halign= 'center', valign = 'middle',x = self.width/2, y= self.height/2)
            self._message = GLabel(text= "Press 's' to start!",font_name= MESSAGE_FONT, font_size = MESSAGE_SIZE,
            halign= 'center', valign = 'middle',x = self.width/2, y= self.height/2 )
            self._image = GImage(source="BlackHole.jpeg",width= 1000, height = 1000,x = 500, y= 500)
    ...
    def update(self,dt):
        ...
        if self._state == STATE_INACTIVE:
            self._message.top = self._title.bottom
            self._checkIfSIsPressed()
            
        if self._state == STATE_LOADING:
            var = GameApp.load_json(DEFAULT_WAVE)
            self._wave = Wave(var)
    ...
    def _resumeGame(self):
        """
        Method that resumes the game after a collison has occured
        
        """
        lives = self._wave.getLives()
        self._message = self._message = GLabel(text= "Press 's' to start! You have " + str(lives) +' lives left',
        font_name= MESSAGE_FONT,
        font_size = MESSAGE_SIZE/2,
        halign= 'center', valign = 'middle',x = self.width/2, y= self.height/2 )
        self._checkIfSIsPressed()

    def _gameOver(self):
        if self._wave.getLives()<= 0:
            self._message = self._message = GLabel(text= "Game Over. You Lost!",font_name= MESSAGE_FONT,
            font_size = MESSAGE_SIZE,
            halign= 'center', valign = 'middle',x = self.width/2, y= self.height/2 )
        else:
            self._message = self._message = GLabel(text= "Congrats! You Won!",font_name= MESSAGE_FONT,
            font_size = MESSAGE_SIZE,
            halign= 'center', valign = 'middle',x = self.width/2, y= self.height/2 )
